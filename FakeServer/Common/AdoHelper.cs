...
namespace FakeServer.Common
{
	...
	public class AdoHelper : IDisposable
	{
		...
		/// <summary>
		/// Constructure using existing connection string
		/// </summary>
		/// <param name="db">own database name</param>
		/// <param name="exist">whether it's existing configuration in web.config file</param>

		public AdoHelper(string db, bool exist)
		{
			if (db == "localhost")
			{
				_connString = "Data Source=localhost;Initial Catalog=Testing;Integrated Security=SSPI;"; // add your connection string or get it from app config
			}
			Connect();
		}
		...
		public SqlCommand CreateCommandFromList(string qry, CommandType type, List<string> args)
		{
			SqlCommand cmd = new SqlCommand(qry, _conn);
			...
			{
				if (args[i] is string && i < (args.Count - 1))
				{
					SqlParameter parm = new SqlParameter();
					parm.ParameterName = (string)args[i];
					parm.Value = args[++i];
		...
		/// <param name="args">Any number of parameter name/value pairs and/or SQLParameter arguments</param>
		/// <returns>Results as a SqlDataReader</returns>
		public SqlDataReader ExecDataReaderGeneric(string qry, List<string> args)
		{
			using (SqlCommand cmd = CreateCommandFromList(qry, CommandType.Text, args))
			{
				return cmd.ExecuteReader();
			}
		}
		...
		/// <param name="args">Any number of parameter name/value pairs and/or SQLParameter arguments</param>
		/// <returns>Results as a SqlDataReader</returns>
		public SqlDataReader ExecDataReaderProcGeneric(string qry, List<string> args)
		{
			using (SqlCommand cmd = CreateCommandFromList(qry, CommandType.StoredProcedure, args))
			{
				return cmd.ExecuteReader();
			}
		}
